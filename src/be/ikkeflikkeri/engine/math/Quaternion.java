...
public class Quaternion
{
	...
	{
		float sinHalfAngle = Mathf.sin(Mathf.toRadians(angle / 2));
		float cosHalfAngle = Mathf.cos(Mathf.toRadians(angle / 2));
	...
	{
		float sinHalfAngle = Mathf.sin(Math.toRadians(angle / 2));
		float cosHalfAngle = Mathf.cos(Math.toRadians(angle / 2));
	...
	public float getYaw()
	{
		return Mathf.toDegrees(Mathf.atan2(2.0f * (y * z + w * x), w * w - x * x - y * y + z * z));
	}
	
	public float getPitch()
	{
		return Mathf.toDegrees(Mathf.asin(-2.0f * (x * z - w * y)));
	}
	
	public float getRoll()
	{
		return Mathf.toDegrees(Mathf.atan2(2.0f * (x * y + w * z), w * w + x * x - y * y - z * z));
	}
	*/
